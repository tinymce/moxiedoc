...
function Builder() {
	...
	/**
	 * Array of types for example classes and enums.
	 *
	 * @member {Array} types
	 */
	this.types = [];
	...
	this.parser.on('end', function() {
		...
		currentBlock.forEach(function(tag) {
			...
			if (Builder.typeTags[tag.name] === true) {
			...
			if (Builder.memberTags[tag.name] === true) {
				callback = Builder.tags[tag.name];
...
Builder.prototype.addType = function(typeInfo) {
...
Builder.prototype.toJSON = function() {
	...
	this.types.forEach(function(typeInfo) {
		json.push(typeInfo.toJSON());
	});
