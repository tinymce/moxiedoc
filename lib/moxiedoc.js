...
exports.process = function(settings) {
	...
	function listFiles(dirPath, patterns) {
		var output = [];

		var matches = function (filePath) {
			return patterns.filter((pattern) => wildcard(pattern, path.basename(filePath))).length > 0;
		};
	...
	settings.paths.forEach(function(filePath) {
		if (fs.statSync(filePath).isDirectory()) {
			listFiles(filePath, ["*.js", "*.ts"]).forEach(function(filePath) {
